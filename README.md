...
특산품 정보 수집을 위해 Selenium 기반의 웹 크롤링을 활용하였다. 네이버 지도를 크롤링하여 다음과 같은 형식의 데이터를 수집하였다:

(1, '부산', '부기인형', '20,000원', '부산 해운대구 해운대해변로 264 바다위구름상점', 35.1591069824231, 129.160283786856, 'https://search.pstatic.net/common/?src=...', 'https://blog.naver.com/risingyou21/223521590171')

상품명, 가격, 주소, 대표 이미지 URL, 후기 블로그 등의 정보를 추출하며,

주소는 도로명 주소를 기준으로 수집하고, 해당 주소를 위경도 좌표로 변환(Geocoding)하여 위치 기반 서비스에 활용할 수 있도록 구성하였다.

이렇게 수집된 데이터는 MySQL 데이터베이스에 저장되며, 이후 클라이언트 요청에 따라 지역별 특산품 정보를 JSON 형태로 응답하도록 설계하였다.
